...
// High-watermark resize: only grow the buffer, never shrink it.
// This prevents repeated alloc/free of differently-sized buffers
// (a prime CUDA memory fragmentation source). After the first few
// frames the tensor stabilises at the maximum R seen so far and
// no new CUDA allocations are made.
std::function<char*(size_t N)> resizeFunctional(torch::Tensor& t) {
    auto lambda = [&t](size_t N) {
        if ((long long)N > t.numel())
	...
	torch::Tensor& imgBuffer)
{
  ...
  // geomBuffer, binningBuffer, imgBuffer are pre-allocated persistent tensors
  // passed in from Python. resizeFunctional uses high-watermark logic: only
  // grows them, never shrinks. After the first large frame they stabilise and
  // no further CUDA allocations occur, eliminating binningBuffer fragmentation.
